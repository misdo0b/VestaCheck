...
export default function ResetPasswordModal({ isOpen, onClose, onSubmit, user }: ResetPasswordModalProps) {
  ...
  const passwordSchema = useMemo(() => z.object({
    password: z.string().min(8, t('adminUsers.validationPasswordMin')),
  }), [t]);
      ...
      <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-sm shadow-2xl shadow-blue-500/10 overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-white/[0.02]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white leading-none mb-0.5">{t('adminUsers.resetPasswordModalTitle')}</h2>
        ...
        {isSuccess ? (
          <div className="p-8 flex flex-col items-center justify-center text-center animate-in zoom-in-95 fill-mode-forwards">
             ...
             <h3 className="text-xl font-bold text-white mb-1">{t('adminUsers.resetPasswordModalSuccessTitle')}</h3>
             <p className="text-slate-400 text-sm">{t('adminUsers.resetPasswordModalSuccessDesc')}</p>
          </div>
        ) : (
          /* Form */
          <form onSubmit={handleSubmit(handleFormSubmit)} className="p-6 space-y-6">
            <div className="space-y-2">
              <div className="flex items-center justify-between ml-1">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('adminUsers.resetPasswordModalSubtitle')}</label>
                <button 
                  type="button" 
                ...
                >
                  ...
                  {t('adminUsers.resetPasswordModalGenerate')}
                </button>
              </div>
              ...
              <div className="relative group">
                <input 
                  ...
                  placeholder={t('adminUsers.resetPasswordModalPlaceholder')}
            ...
            <p className="text-[11px] text-slate-500 bg-white/5 p-3 rounded-lg border border-white/5 italic">
              {t('adminUsers.resetPasswordModalWarning')}
            </p>
