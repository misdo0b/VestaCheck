      ...
      <div className="flex items-center gap-1.5 border-r border-white/10 pr-3 mr-1">
        {isOnline ? (
          <>
            ...
            <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider">{t('sync.online')}</span>
          </>
        ) : (
          <>
            <WifiOff className="w-4 h-4 text-orange-500" />
            <span className="text-[10px] font-bold text-orange-500 uppercase tracking-wider">{t('sync.offline')}</span>
          </>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        {isSyncing ? (
          <>
            ...
            <span className="text-[11px] text-slate-300 font-medium">{t('sync.syncing')}</span>
          </>
        ) : isOnline ? (
          <button 
          ...
          >
            ...
            <span className="text-[11px] text-slate-400 font-medium group-hover:text-slate-200">{t('sync.synced')}</span>
          </button>
        ) : (
          <>
            <CloudOff className="w-4 h-4 text-slate-500" />
            <span className="text-[11px] text-slate-500 font-medium italic">{t('sync.waitingNetwork')}</span>
          </>
        )}
      </div>
    </div>
