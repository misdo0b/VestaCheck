...
export const generatePDF = async (
  ...
  try {
    ...
    const tenantName = tenant?.name || activeData.manualTenant?.name || 'Locataire inconnu';
    ...
    pdf.text(`${activeData.type.toUpperCase()} • ${activeData.date}`, pdfWidth - margin, 22, { align: 'right' });
    ...
    pdf.text("1. RELEVÉ DES COMPTEURS", margin, currentY);
    currentY += 4;

    const counterBody = [
      ['Eau potable', String(activeData.counters?.water ?? 0), 'm³'],
      ['Électricité', String(activeData.counters?.electricity ?? 0), 'kWh']
    ];
    if (activeData.counters?.gas !== undefined && activeData.counters?.gas !== null) {
      counterBody.push(['Gaz naturel', String(activeData.counters.gas), 'm³']);
    }

    autoTable(pdf, {
      startY: currentY,
      margin: { left: margin, right: margin },
      head: [['Type de Compteur', 'Valeur Relevée', 'Unité']],
      body: counterBody,
      theme: 'striped',
    ...
    pdf.setFontSize(11);
    pdf.setTextColor(17, 24, 39);
    pdf.text("2. INVENTAIRE DES CLÉS ET BADGES", margin, currentY);
    currentY += 4;

    const keysBody = (activeData.keyInventories || []).map(k => [k.type, String(k.count ?? 0)]);
    if (keysBody.length === 0) {
      keysBody.push(['Aucun trousseau ou badge répertorié.', '-']);
    }

    autoTable(pdf, {
      startY: currentY,
      margin: { left: margin, right: margin },
      head: [['Désignation du trousseau / clé', 'Nombre']],
      body: keysBody,
      theme: 'striped',
      headStyles: { fillColor: [37, 99, 235], textColor: [255, 255, 255], fontStyle: 'bold' },
      styles: { fontSize: 9.5, cellPadding: 5 },
    ...
    pdf.setFontSize(11);
    pdf.setTextColor(17, 24, 39);
    pdf.text("3. DESCRIPTIF DÉTAILLÉ DES PIÈCES", margin, currentY);
    currentY += 6;
    ...
    if (rooms.length === 0) {
      ...
      pdf.text("Aucune pièce répertoriée dans ce rapport.", margin, currentY);
      currentY += 10;
    } else {
      for (const room of rooms) {
        ...
        autoTable(pdf, {
          startY: currentY,
          margin: { left: margin, right: margin },
          head: [['Élément', 'État', 'Observations / Commentaires']],
          body: itemsBody,
          theme: 'striped',
          ...
          didParseCell: (cellData) => {
            ...
            if (cellData.column.index === 1 && cellData.section === 'body') {
              const cond = String(cellData.cell.text[0] || '').trim();
              if (cond === 'Neuf') {
    ...
    if (activeData.generalObservations && activeData.generalObservations.trim() !== '') {
      ...
      pdf.setFontSize(11);
      pdf.setTextColor(17, 24, 39);
      pdf.text("OBSERVATIONS GÉNÉRALES", margin, currentY);
      currentY += 4;
    ...
    pdf.setFontSize(11);
    pdf.setTextColor(17, 24, 39);
    pdf.text("VALIDATION & SIGNATURES", margin, currentY);
    currentY += 4;
    ...
    autoTable(pdf, {
      startY: currentY,
      margin: { left: margin, right: margin },
      head: [['Le Locataire', "L'Inspecteur"]],
      body: [
        ['', ''], // Ligne vide réservée aux images vectorielles des signatures
        [
          `${tenantName}\n${activeData.signatures?.tenant?.signedAt ? 'Signé le ' + new Date(activeData.signatures.tenant.signedAt).toLocaleDateString('fr-FR') + ' à ' + new Date(activeData.signatures.tenant.signedAt).toLocaleTimeString('fr-FR') : 'Non signé'}`,
          `${inspectorName}\n${activeData.signatures?.inspector?.signedAt ? 'Signé le ' + new Date(activeData.signatures.inspector.signedAt).toLocaleDateString('fr-FR') + ' à ' + new Date(activeData.signatures.inspector.signedAt).toLocaleTimeString('fr-FR') : 'Non signé'}`
        ]
      ],
      theme: 'plain',
      ...
      didDrawCell: (cellData) => {
        if (cellData.row.index === 0 && cellData.section === 'body') {
          ...
          if (sigBase64 && sigBase64.startsWith('data:image')) {
            ...
            pdf.text('Non signé ou en attente', cellData.cell.x + cellData.cell.width / 2, cellData.cell.y + 13, { align: 'center' });
          }
        }
      }
    });
    ...
    if (allPhotos.length > 0) {
      ...
      pdf.setFontSize(14);
      pdf.setTextColor(17, 24, 39);
      pdf.text("4. ANNEXE PHOTOGRAPHIQUE", margin, 25);
      ...
      allPhotos.forEach((photoInfo, idx) => {
        ...
        if (photoSrc) {
          ...
          try {
            ...
            pdf.setFontSize(8);
            pdf.setTextColor(156, 163, 175);
            pdf.text("Cliché indisponible", x + imgWidth / 2, y + imgHeight / 2, { align: 'center' });
          }
        } else {
          pdf.setFillColor(249, 250, 251);
          pdf.rect(x + 0.5, y + 0.5, imgWidth - 1, imgHeight - 1, 'F');
        ...
        pdf.setFontSize(8);
        pdf.setTextColor(17, 24, 39);
        pdf.text(`PIÈCE : ${photoInfo.roomName.toUpperCase()}`, x, labelY);
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      if (i > 1) {
        ...
        pdf.setFont('Helvetica', 'normal');
        pdf.text(`${activeData.type.toUpperCase()} • ${activeData.date} • ${tenantName}`, pdfWidth - margin, 12, { align: 'right' });
